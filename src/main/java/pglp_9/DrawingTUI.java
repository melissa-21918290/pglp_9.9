...
public class DrawingTUI {
	...
	public Command nextCommand(String cmd) {
        ...
        } else if (cmd.contains("deplacer")) {
            return this.depalcer(cmd);
        } else if (cmd.contains("supprimer")) {
	...
	public Forme creatRectangle(final String variable, final String[] split2) {
        final int quatre = 4;
            ...
            if (split.length != quatre) {
                ...
                try {
                    ...
                    return new Rectangle(
                            variable, topLeft, longueur, largeur);
                } catch (Exception e) {
                    System.err.println("Commande invalide, "
                            + "impossible de créer la forme");
                }
...
private Forme create(final String cmd2) {
    ...
    if (split[0].contains(" ")) {
        ...
        if (split[1].contains("Cercle")) {
            ...
            f = this.creatRectangle(variable, split);
...
private Command depalcer(final String cmd2){
	
        final int trois = 3;
        String cmd = cmd2.replace(" ", "");
        String[] split = cmd.split("deplacer");
        if (!split[0].equals("")
                || !(split[1].startsWith("(") && split[1].endsWith(")"))) {
            System.err.println("Commande invalide, parenthèses manquantes");
        } else {
...
private Command supprimer(final String cmd2){
	 String cmd = cmd2.replace(" ", "");
     String[] split = cmd.split("supprimer");
     if (!split[0].equals("")
             || !(split[1].startsWith("(") && split[1].endsWith(")"))) {
         System.err.println("Commande invalide, parenthèses manquantes");
     } else {
